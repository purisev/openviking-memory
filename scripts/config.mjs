...
export function loadConfig(cwd = process.cwd()) {
  ...
  return {
    ...
    serverAutoCommit: envBool("OPENVIKING_SERVER_AUTO_COMMIT") ?? (cx.serverAutoCommit === true),
    commitTokenThreshold: Math.max(1000, Math.min(1000000, Math.floor(num(
      process.env.OPENVIKING_COMMIT_TOKEN_THRESHOLD,
      num(cx.commitTokenThreshold, 20000),
    )))),
    commitKeepRecentCount: Math.max(0, Math.min(1000, Math.floor(num(
